...
<html>
	...
	<body>
		<div class="body-box">
			...
			<form name="tableForm" method="post" action="accountTrade_queryAccountToQueryAccountTrde.do" onsubmit="return check();" >
				<table width="100%" class="pn-ftable" cellpadding="2" cellspacing="1" border="0">
					...
					<tr>
						...
						<td colspan="3" class="pn-fcontent" >
						  <input type="text" id="startTime" name="startTime" value="<fmt:formatDate value="${startTime}" pattern="yyyy-MM-dd HH:mm:ss" />" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})""> - <input type="text" id="endTime" name="endTime" value="<fmt:formatDate value="${endTime}" pattern="yyyy-MM-dd HH:mm:ss" />" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"">
					      <input type="checkbox" name="unusualAccountTrade" value="1" <c:if test="${unusualAccountTrade==1 }"> checked="checked" </c:if>/>异常流水查询
						</td>
					    </tr>
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${list}" var="item">
						<tr onmouseover="this.className='pn-lhover'" onmouseout="this.className=''" align="center">
							...
							<!--<c:if test="${sysparamconf2.pvalue==1}">
							<td><fmt:formatNumber value="${item.cardBalance/100}" pattern="#.##" type="number"/>元</td>
							</c:if>-->
							<td><a href="accountTrade_detail.do?account.sys_id=${item[0]}&accountTrade.listNo=${item[8]}&accountTrade.opTime=${item[4]}" class="pn-loperator">详细信息</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
