...
<html>
	<head>
		...
		<script type="text/javascript">
			if("${result}"=="FAIL")
			  ...
			  function add(){
		      window.location="userCarAccount_add.do";
		       }
	...
	<body>
		<div class="body-box">
			...
			<form name="tableForm" method="post" action="userCarAccount_queryAccountToAdd.do" onsubmit="return check();">
					...
					<tr>					  
						...
						<td  class="pn-fcontent">  
							<select name="user.credentialsType" style="width:150px">
								<option value="" <c:if test="${user.credentialsType==null}">selected</c:if>>请选择</option>
							    <option value="0" <c:if test="${user.credentialsType==0}">selected</c:if>>身       份            证</option>
								<option value="1" <c:if test="${user.credentialsType==1}">selected</c:if>>其                     他 </option>
							</select>
