...
<html>
		...
		<script type="text/javascript">
			if("${result}"=="FAIL")
		       ...
		       function check(){
		        ...
		        function blackOut(sys_id){
		        if(confirm("确定要把此账户移出黑名单吗？")) {
		       			 window.location="userCarAccount_blackOutOne.do?sys_id="+sys_id+"&account.sys_id="+get("account.sys_id").value+"&account.cardNo="+get("account.cardNo").value+"&user.userName="+get("user.userName").value+"&car.vehPlate="+get("car.vehPlate").value+"&user.credentialsNo="+get("user.credentialsNo").value+"&account.status="+get("account.status").value;
	...
	<body>
		<div class="body-box">
			...
			<form name="tableForm" method="post" action="userCarAccount_queryAccountToUpdate.do" onsubmit="return check();">
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${list}" var="item">
						<tr onmouseover="this.className='pn-lhover'" onmouseout="this.className=''" align="center">
							...
							<td><a href="javascript:blackOut(${item.sys_id})" class="pn-loperator">移出黑名单</a></td>					   
						    </c:if>
							
							
							
                            <c:if test="${(item.status==0||item.status==3||item.status==6)}"> 
                               <td><a href="userCarAccount_accountList.do?account.sys_id=${item.sys_id}" class="pn-loperator">修改帐户信息</a></td>
                            </c:if>
                            
                            <c:if test="${(item.status!=0&&item.status!=3&&item.status!=6&&item.status!=7)}"> 
                            <td></td>
                            </c:if>
