		  ...
		  function saveAndPrint(){
			...
			if(countDay > 10) {
		  	...
		  	if(check() == false) {
			    ...
			    window.open("otherTollsReport_payCardTradeReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value+"&tradeType="+ids
		      	...
		      	//window.open("otherTollsReport_payCardTradeReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value+"&tradeType="+ids,"","location=no,resizable=yes,scrollbars=yes,height=600,width=800,top=0,left=0");
		    }
		  }
		...
		//生成当前页面HTML报表
		  function saveAndPrintCurrentPage(){
				...
				if(countDay > 10) {
			  	...
			  	if(check() == false) {
					...
					window.belowFrame.location.href="otherTollsReport_payCardTradeReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value+"&tradeType="+ids
				    	+"&account.cardNo="+get("account.cardNo").value+"&account.status="+get("account.status").value
			    		+"&car.vehPlate="+get("car.vehPlate").value+"&user.credentialsNo="+get("user.credentialsNo").value
			    		+"&user.userName="+get("user.userName").value+"&account.sys_id="+get("account.sys_id").value;
			    }
		  ...
		  function saveAndPrintPDF() {
		  	...
		  	if(check() == false) {
		    	...
		    	window.open("otherTollsReport_payCardTradeReportPDF.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value+"&tradeType="+ids
		    		+"&account.cardNo="+get("account.cardNo").value+"&account.status="+get("account.status").value
		    		+"&car.vehPlate="+get("car.vehPlate").value+"&user.credentialsNo="+get("user.credentialsNo").value
		    		+"&user.userName="+get("user.userName").value+"&account.sys_id="+get("account.sys_id").value);
		    }
