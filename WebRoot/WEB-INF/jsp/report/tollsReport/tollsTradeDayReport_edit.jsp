...
<html>
	<head>
		...
		<script type="text/javascript">
		  window.onload = function() {
		  	  ...
		  	  //var tab_height = get("tab").offsetHeight;
		  }
		  function changeSize() {
		  	  var ifr_height = $(window).height() - $("#div1").height() - $("#div2").height() - 30;
		  	  var iframe = $("#iframe").height(ifr_height);
		  ...
		  function saveAndPrint(){
			...
			if(countDay > 10) {
		  	...
		  	if(checkTime() == false) {
				...
				window.open("tollsReport_tollsTradeDayReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		      	//window.open("tollsReport_tollsTradeDayReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value,"","location=no,resizable=yes,scrollbars=yes,height=600,width=800,top=0,left=0");
		    }
		  }
			  ...
			  var countDay = compTime(get("startTime").value, get("endTime").value);
				if(countDay > 10) {
			  	...
			  	if(checkTime() == false) {
					...
					window.belowFrame.location.href="tollsReport_tollsTradeDayReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value;
			    }
		  }
		  //生成PDF报表
		  function saveAndPrintPDF() {
		  	if(checkTime() == false) {
		    	...
		    	window.open("tollsReport_tollsTradeDayReportPDF.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		    }
		  }
		  //校验日期
		  function checkTime() {
	...
	<body onResize="changeSize()">
		<div class="body-box">
			...
			<div id="div2">
				<table id="tab" width="100%" class="pn-ftable" cellpadding="10" cellspacing="1" border="0">
					<tr>
					...
					<tr align="center">
						<td style="padding-top: 5px;padding-bottom: 5px;"  colspan="2" class="pn-fcontent">
					      ...
					      <input type="button" value="&nbsp;查&nbsp;&nbsp;询&nbsp;" onclick="return saveAndPrintCurrentPage()"></td>
					</tr>
				</table>
			</div>
