	...
	<head>
		...
		<script type="text/javascript">
		  ...
		  function saveAndPrint(){
			...
			if(countMonth > 10) {
				if(confirm("所选月份超过10月，采用HTML方式生成报表可能需要较长时间，是否采用PDF方式生成报表？")) {
				  	saveAndPrintPDF();
				  	return true;
			  	}
		  	} 
		  	if(checkTime() == false) {
			    ...
			    window.open("trafficFlowReport_trafficFlowMonthReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		      //window.open("trafficFlowReport_trafficFlowMonthReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value,"","location=no,resizable=yes,scrollbars=yes,height=600,width=800,top=0,left=0");
		    }
		  }
		...
		//生成当前页面HTML报表
		  function saveAndPrintCurrentPage(){
				...
				if(countMonth > 10) {
			  	...
			  	if(checkTime() == false) {
					...
					window.belowFrame.location.href="trafficFlowReport_trafficFlowMonthReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value;
			    }
		  }
		  ...
		  function saveAndPrintPDF() {
		  	if(checkTime() == false) {
		    	...
		    	window.open("trafficFlowReport_trafficFlowMonthReportPDF.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		    }
		  }
		  //校验日期
		  function checkTime() {
