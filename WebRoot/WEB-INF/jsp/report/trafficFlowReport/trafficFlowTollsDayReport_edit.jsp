		  ...
		  function saveAndPrint() {
			...
			if(countDay > 10) {
		  	...
		  	if(checkTime() == false) {
			    ...
			    window.open("trafficFlowReport_trafficFlowTollsDayReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		      //window.open("trafficFlowReport_trafficFlowTollsDayReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value,"","location=no,resizable=yes,scrollbars=yes,height=600,width=800,top=0,left=0");
		    }
		  }
		...
		//生成当前页面HTML报表
		  function saveAndPrintCurrentPage(){
				...
				if(countDay > 10) {
			  	...
			  	if(checkTime() == false) {
					...
					window.belowFrame.location.href="trafficFlowReport_trafficFlowTollsDayReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value;
			    }
		  }
		  ...
		  function saveAndPrintPDF() {
		  	if(checkTime() == false) {
		    	...
		    	window.open("trafficFlowReport_trafficFlowTollsDayReportPDF.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		    }
		  }
		  //校验日期
		  function checkTime() {
