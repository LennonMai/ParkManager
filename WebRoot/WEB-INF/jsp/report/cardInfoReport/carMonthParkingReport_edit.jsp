		...
		<script type="text/javascript">
		  ...
		  function saveAndPrint(){
			...
			if(countDay > 10) {
		  	...
		  	if(checkTime() == false) {
			    ...
			    window.open("cardInfoReport_carMonthParkingReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		    }
		  }
		  //生成PDF报表
		  function saveAndPrintPDF(){
		  	if(checkTime() == false) {
		    	...
		    	window.open("cardInfoReport_carMonthParkingReportPDF.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		    }
		  }
		  //生成EXCEL报表
		  function saveAndPrintExcel(){
		  	if(checkTime() == false) {
			  	return false;
			} else {
		    	window.open("cardInfoReport_carMonthParkingReportExcel.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value);
		    }
		  }
		  //生成当前页面HTML报表
		  function saveAndPrintCurrentPage(){
				...
				if(countDay > 10) {
			  	...
			  	if(checkTime() == false) {
					...
					window.belowFrame.location.href="cardInfoReport_carMonthParkingReport.do?startTime="+get("startTime").value+"&endTime="+get("endTime").value;
			    }
		  }
		  //校验日期
		  function checkTime() {
				...
				<table width="100%" class="pn-ftable" cellpadding="2" cellspacing="1" border="0">
					...
					<tr align="center">
						<td style="padding-top: 5px;padding-bottom: 5px;" colspan="2" class="pn-fcontent">
						  ...
						  <input type="button" value="生成EXCEL报表" onclick="return saveAndPrintExcel()"/>
