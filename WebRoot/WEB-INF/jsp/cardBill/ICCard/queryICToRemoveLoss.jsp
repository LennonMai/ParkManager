...
<html>
	...
	function removeLoss(cardNo) {	
		if(confirm("确定要解挂吗？")) {	
			var v="succuss";
			jQuery.ajax({type:"post",url:"iCCard_ajaxgetStoreCardBlackListByCardNoToReMoveLoss.do?cardInfo.cardNo=" +cardNo,async:false, success:function(data,textStatus){
			 	if(data=="secondUseTwo") {
					 	v="No";alert("该卡之前已被运用过，不能再运用"); 
					}			   
				}
			});
			if(v!="No"){
				window.location = "iCCard_removeLoss.do?cardInfo.cardNo=" + cardNo;	
			}	
	...
	<body>
		<div class="body-box">
			...
			<form name="tableForm" method="post" action="iCCard_queryICToRemoveLoss.do">
					...
					<c:forEach items="${list}" var="item">
						<tr onmouseover="this.className='pn-lhover'" onmouseout="this.className=''" align="center">
                            ...
                            <c:when test="${item[0].status==1}"> 
                               <td><a href="javascript:removeLoss('${item[0].cardNo}')" class="pn-loperator">解挂</a></td>
                             </c:when>                         
                             <c:otherwise> 
                              <td/> &nbsp;</td>
                             </c:otherwise> 
