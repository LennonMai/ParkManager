...
<html>
	<head>
		...
		function ishasSquadNo() {
			var squadNo = $("#squadNo").val();
			jQuery.post("baseInfo_ishasSquadNo.do?squadInformation.squadNo="+squadNo, function(data) {
				if(data == "hasSquadNo") {
					$("#squadNo").val("");
		    		$("#squadNo").focus();
					alert("工班编号已存在！");
		...
		//caijunhua jquery异步验证工班名称是否存在
		function getSquadByName() {
			var squadName = $("#squadName").val();
			jQuery.post("baseInfo_getSquadByName.do?squadInformation.squadName="+squadName, function(data) {
				if(data == "fail") {
					$("#squadName").val("");
		    		$("#squadName").focus();
					alert("工班名称已存在！");
	...
	<body>
		<div class="body-box">
			...
			<form id="dataform" method="post" action="baseInfo_saveSquadInfo.do">
				<table width="100%" class="pn-ftable" cellpadding="2" cellspacing="1" border="0">
                    <tr>
						<td class="pn-flabel pn-flabel-h">工班编号：</td>
						<td class="pn-fcontent"><input id="squadNo" type="text" class="required isNo_Tinyint" name="squadInformation.squadNo" value="${squadInformation.squadNo}" size="28" onblur="ishasSquadNo() ;"/><span style="color: red">&nbsp;*</span></td>
			    		<td class="pn-flabel pn-flabel-h">工班名称：</td>
						<td class="pn-fcontent"><input type="text" id="squadName" class="required maxlength20" onblur="getSquadByName();"  name="squadInformation.squadName" value="${squadInformation.squadName}" size="28"/><span style="color: red">&nbsp;*</span></td>
					    </tr>
					<tr>
