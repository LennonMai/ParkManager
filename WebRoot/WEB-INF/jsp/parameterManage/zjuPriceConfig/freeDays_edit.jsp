					...
					<tr>
						...
						<select style="width: 185px;height: 22px;" id="freeDays_type" name="freeDays.type" class="required ">
							<c:if test="${freeDays.type == 0}">
								<option value="0" selected="selected">0 -星期</option>
							</c:if>
						...
						<td colspan="4" class="pn-fcontent">
							<c:if test="${freeDays.type == 0}">
								<select style="width: 185px;height: 22px;" id="freeWeek" name="freeWeek">
									<option value="1" <c:if test="${freeDays.value eq '1         ' }"> selected="selected" </c:if>>星期一</option>
									<option value="2" <c:if test="${freeDays.value eq '2         ' }"> selected="selected" </c:if>>星期二</option>
									<option value="3" <c:if test="${freeDays.value eq '3         ' }"> selected="selected" </c:if>>星期三</option>
									<option value="4" <c:if test="${freeDays.value eq '4         ' }"> selected="selected" </c:if>>星期四</option>
									<option value="5" <c:if test="${freeDays.value eq '5         ' }"> selected="selected" </c:if>>星期五</option>
									<option value="6" <c:if test="${freeDays.value eq '6         ' }"> selected="selected" </c:if>>星期六</option>
									<option value="7" <c:if test="${freeDays.value eq '7         ' }"> selected="selected" </c:if>>星期日</option>
								</select>
							</c:if>
							<c:if test="${freeDays.type == 1}">
