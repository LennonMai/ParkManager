...
<html>
	<head>
		...
		<script type="text/javascript">
			if("${result}"=="FAIL")
				alert("帐号状态为挂失状态才能办理解挂");
				if("${result}"=="FAIL1")
				alert("帐号状态为预销户（无效）状态，不能办理解挂");
				if("${result}"=="FAIL2")
				alert("帐号状态为黑名单（无效）状态，不能办理解挂");
				if("${result}"=="FAIL3")
				alert("帐号状态为其他（无效）状态，不能办理解挂");
				if("${result}"=="FAIL6")
				alert("帐号状态为已销户（无效）状态，不能办理解挂");
				if("${result}"=="FAIL4")
				alert("没有此信息账户，请重新输入");			 			 
		        function check(){
              ...
              function removeLoss(sys_id){
		       	if(confirm("确定要解挂吗？")) {
		       		window.location="account_removeLoss.do?account.sys_id="+sys_id;
		       	}
		      }							
		</script>		
	</head>
	<body>
		<div class="body-box">
			...
			<form name="tableForm" method="post" action="account_queryAccountToRemoveLoss.do" onsubmit="return check();" >
					...
					<c:forEach items="${list}" var="item">
						<tr onmouseover="this.className='pn-lhover'" onmouseout="this.className=''" align="center">
                            ...
                            <c:when test="${item.status==1}"> 
                              <td><a href="javascript:removeLoss(${item.sys_id})" class="pn-loperator" >解挂</a></td>
                             </c:when>                            
                             <c:otherwise> 
                              <td/> &nbsp;</td>
                             </c:otherwise> 
