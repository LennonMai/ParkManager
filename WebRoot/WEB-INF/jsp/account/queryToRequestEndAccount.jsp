...
<html>
	<head>
		...
		<script type="text/javascript">
			if("${result}"=="FAIL")
				alert("帐号状态为挂失（无效）状态，不能办理申请销户");
				if("${result}"=="FAIL1")
				alert("帐号状态已为预销户（无效）状态，不能再办理申请销户");
				if("${result}"=="FAIL2")
				alert("帐号状态为黑名单（无效）状态，不能办理申请销户");
				if("${result}"=="FAIL3")
				alert("帐号状态为其他（无效）状态，不能办理申请销户");
				if("${result}"=="FAIL6")
				alert("帐号状态为已销户（无效）状态，不能办理申请销户");
				if("${result}"=="FAIL4")
				alert("没有此信息账户，请重新输入");						 
		        function check(){
			 ...
			 function requestEndAccount(sys_id){
		       		if(confirm("确定要申请销户吗？")) {
		       			window.location="account_requestEndAccount.do?account.sys_id="+sys_id;
		       		}
		       }
	...
	<body>
		<div class="body-box">
			...
			<form name="tableForm" method="post" action="account_queryAccountToRequestEndAccount.do" onsubmit="return check();" >
						...
						<tr onmouseover="this.className='pn-lhover'" onmouseout="this.className=''" align="center">
							...
							<c:choose> 
                             <c:when test="${item.status==0||item.status==3||item.status==6}"> 
                              <td><a href="javascript:requestEndAccount(${item.sys_id})" class="pn-loperator">申请销户</a></td>
                             </c:when>                             
                             <c:otherwise> 
                              <td/> &nbsp;</td>
                             </c:otherwise> 
