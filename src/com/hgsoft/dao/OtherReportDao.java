...
public class OtherReportDao {
	@Resource
	SessionFactory otherSessionFactory;
	
	public Session getSession(){
		//System.out.println("---------->"+otherSessionFactory.toString());
	...
	public JasperPrint fillReport(String sourceFile,Map<String,Object> parameters) throws JRException{
		return JasperFillManager.fillReport(sourceFile,parameters,otherSessionFactory.getCurrentSession().connection());
	}
	/**
	 * 生成Stream形式的PDF格式报表
	...
	public void PdfStreamFormate(InputStream inputStream,OutputStream outStream,Map<String,Object> parameters,int conn) throws JRException{
		if(conn == 0)
		  ...
		  JasperRunManager.runReportToPdfStream(inputStream,outStream,parameters,otherSessionFactory.getCurrentSession().connection());
		}
	}
	...
	public String pdfOrHtmlFormat(String sourceFile,Map<String,Object> parameters,int format) throws JRException{
		if(format == 0)
		  return JasperRunManager.runReportToPdfFile(sourceFile, parameters, otherSessionFactory.getCurrentSession().connection());	
		else if(format == 1)
		  return JasperRunManager.runReportToHtmlFile(sourceFile, parameters, otherSessionFactory.getCurrentSession().connection());
		else
		  return null;
	}
	...
	//计算生成通行费分车道分时段报表记录条数
		public int countTollsLane(String startTime, String endTime, int areaNoValue, int parkNoValue,String pLaneNo) {
			StringBuffer sf = new StringBuffer("from tb_OutListSum as a left join tb_HandInMoney as b " +
					...
					"where a.AreaNo=? and a.ParkNo=? and a.SquadDate between ? and ? and a.LaneNo in("+pLaneNo+") " +
					"group by a.LaneNo,a.SquadDate ");
			List<Object> obj = new ArrayList<Object>();
			obj.add(areaNoValue);
			obj.add(parkNoValue);
			obj.add(startTime);
